...
const Flower: React.FC<{ className?: string; style?: React.CSSProperties }> = ({ className, style }) => (
  <svg viewBox="0 0 80 80" className={className} style={style} fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    {[...Array(20)].map((_, i) => {
      ...
      const radius = 6 + (i % 3);
      return <circle key={`seed-${i}`} cx={40 + Math.cos(angle) * radius} cy={40 + Math.sin(angle) * radius} r="1.2" fill="#5D4037" />;
    })}
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const checkEnvironment = () => {
      ...
      // Target bounds to avoid scrolling
      const targetBaseHeight = 780;
      const targetBaseWidth = height > width ? 380 : 700;
      
      const hScale = height / targetBaseHeight;
      const wScale = width / targetBaseWidth;
      
      // We want to ensure it fits in both dimensions
  ...
  const renderScreen = () => {
    ...
    switch (screen) {
      case AppScreen.OPENING:
        return (
          <div className="flex flex-col items-center text-center px-2 animate-in fade-in duration-700">
            ...
            <button onClick={() => handleScreenTransition(AppScreen.BUILDUP)} className="bg-[#fbc02d] text-[#5d4037] px-8 py-3 rounded-full text-xl md:text-2xl hand-drawn-border hover:bg-[#fdd835] transition-all transform hover:scale-105 active:scale-95 shadow-lg flex items-center gap-3">
      ...
      case AppScreen.BUILDUP:
        return (
          <div className="flex flex-col items-center text-center px-2 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <Bee style={{ width: iconSize, height: iconSize }} className="mb-4 floating" />
            ...
            <button onClick={() => handleScreenTransition(AppScreen.THE_ASK)} className="bg-[#c5e1a5] text-[#5d4037] px-8 py-3 rounded-full text-xl md:text-2xl hand-drawn-border hover:bg-[#dcedc8] transition-all transform hover:scale-105 active:scale-95 shadow-lg flex items-center gap-3">
              <span>🐝</span> Go on…
            </button>
          </div>
        );
      case AppScreen.THE_ASK:
        return (
          <div className="flex flex-col items-center text-center px-2 animate-in zoom-in duration-500">
            <HoneyPot style={{ width: iconSize * 1.2, height: iconSize * 1.4 }} className="mb-2 floating" />
            ...
            <div className="flex flex-col gap-2 w-full max-w-xs md:max-w-sm">
              <button onClick={() => handleScreenTransition(AppScreen.SUCCESS)} className="bg-[#fbc02d] text-[#5d4037] py-2.5 md:py-4 rounded-full text-lg md:text-xl hand-drawn-border hover:bg-[#fdd835] transition-all shadow-md"><span>🍯</span> Yes (this feels correct)</button>
              <button onClick={() => handleScreenTransition(AppScreen.SUCCESS)} className="bg-[#fff176] text-[#5d4037] py-2.5 md:py-4 rounded-full text-lg md:text-xl hand-drawn-border hover:bg-[#fff59d] transition-all shadow-md"><span>🌻</span> Yes (with lots of food!)</button>
              <button onClick={() => handleScreenTransition(AppScreen.SUCCESS)} className="bg-[#ffd54f] text-[#5d4037] py-2.5 md:py-4 rounded-full text-lg md:text-xl hand-drawn-border hover:bg-[#ffca28] transition-all shadow-md"><span>🐝</span> Yes (forever and always)</button>
            </div>
          </div>
        );
      case AppScreen.SUCCESS:
        return (
          <div className="flex flex-col items-center text-center px-2 z-10 animate-in fade-in zoom-in-50 duration-1000">
            <div className="flex gap-4 mb-1 items-end">
              <HoneyPot style={{ width: iconSize * 0.7, height: iconSize * 0.9 }} className="floating" />
              <Bee style={{ width: iconSize * 0.4, height: iconSize * 0.4 }} className="animate-bounce" />
            </div>
            <h1 className="text-2xl md:text-4xl text-[#5d4037] mb-1 storybook-font font-bold">Oh bother… how wonderful!</h1>
            <p className="text-lg md:text-xl text-[#5d4037] mb-3 leading-relaxed max-w-md font-bold">I’m very glad I asked.<br/>I promise to share my honey.</p>
            <div className="bg-[#fffde7] p-4 rounded-2xl hand-drawn-border shadow-md mb-4 max-w-sm w-full border-[#d4a017]">
            ...
            <div className="p-3 bg-white/60 rounded-xl border border-dashed border-[#fbc02d] storybook-font text-base text-[#5d4037] max-w-md shadow-inner italic leading-snug">{POOH_QUOTE}</div>
          </div>
        );
    }
  };
  ...
  return (
    <div className="fixed inset-0 w-full h-full flex items-center justify-center overflow-hidden cursor-default" onClick={() => { setClickCount(c => c + 1); resetIdleTimer(); }}>
      ...
      {/* Container with lower opacity (70%) and no overscroll */}
      <div 
        className="relative z-10 w-[94%] max-w-lg bg-white/70 backdrop-blur-md rounded-[2.5rem] hand-drawn-border shadow-2xl transition-all duration-700 no-scrollbar overflow-hidden" 
        style={{ 
          transform: `scale(${scaleFactor})`, 
          transformOrigin: 'center center', 
          padding: isPortrait ? '1.5rem 0.75rem' : '2.5rem',
          maxHeight: '94vh'
        }}
      >
        {renderScreen()}
      </div>
      <div className={`fixed ${isPortrait ? 'top-10' : 'bottom-10'} left-0 w-full text-center pointer-events-none px-4 z-20`}>
        {idleText && <p className="text-white text-xl md:text-3xl animate-bounce storybook-font font-bold drop-shadow-[0_2px_4px_rgba(0,0,0,0.8)]">{idleText}</p>}
      </div>
    </div>
  );
};
