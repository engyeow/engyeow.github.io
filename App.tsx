...
const App: React.FC = () => {
  ...
  const calculateScale = useCallback(() => {
    ...
    // We target a safe area for our content to avoid any clipping.
    // Portrait users need a bit more vertical breathing room.
    const isPortrait = height > width;
    
    // Base dimensions we want to guarantee fit on screen.
    const baseWidth = isPortrait ? 450 : 700;
    const baseHeight = isPortrait ? 750 : 600;
    
    const wScale = width / baseWidth;
    const hScale = height / baseHeight;
    
    // Calculate the best scale factor to fit the container.
    // We cap it at 1.0 so it doesn't get awkwardly huge on giant monitors.
    let newScale = Math.min(wScale, hScale, 1.0);
    
    // Ensure it doesn't get microscopically small either.
      ...
      case AppScreen.SUCCESS:
        return (
          <div className="flex flex-col items-center text-center px-4 z-10 animate-in fade-in zoom-in-50 duration-1000">
            ...
            <p className="text-xl md:text-2xl text-[#5d4037] mb-6 leading-relaxed max-w-md font-bold">
              I’m very glad I asked.<br/>
              I promise to be kind, and to share my honey.
            </p>
            
            <div className="bg-[#fffde7] p-6 rounded-[2rem] hand-drawn-border shadow-md mb-8 max-w-sm w-full border-[#d4a017]">
              <div className="text-[#d4a017] text-4xl mb-2">🔥</div>
              <h3 className="text-[#5d4037] text-2xl font-bold storybook-font mb-2">Our Date Invitation:</h3>
              <p className="text-[#5d4037] text-xl font-bold leading-relaxed">
            ...
            <div className="p-5 bg-white/60 rounded-2xl border border-dashed border-[#fbc02d] storybook-font text-xl text-[#5d4037] max-w-md shadow-inner italic">
              {POOH_QUOTE}
            </div>
          </div>
        );
  ...
  return (
    ...
    >
      ...
      {isInAppBrowser && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 z-[110] bg-[#fffde7]/90 px-6 py-2 rounded-full border-2 border-[#fbc02d] shadow-lg pointer-events-none">
          <p className="text-[#5d4037] text-sm md:text-base storybook-font font-bold whitespace-nowrap">
             🍯 Tip: Open in Safari or Chrome for the best experience!
          </p>
        </div>
      )}
      ...
      {/* Main Box - Perfectly Centered with Scale Transform */}
      <div 
        className="relative z-10 w-[90%] max-w-[600px] bg-white/90 backdrop-blur-md rounded-[3rem] hand-drawn-border shadow-2xl transition-all duration-700 flex flex-col items-center justify-center overflow-y-auto"
        style={{ 
      ...
      {/* Idle / Secret Text */}
      <div className="fixed bottom-12 left-0 w-full text-center pointer-events-none px-4 z-20">
        {idleText && (
          <p className="text-white text-2xl md:text-4xl animate-bounce storybook-font font-bold drop-shadow-[0_4px_6px_rgba(0,0,0,0.8)]">
            {idleText}
          </p>
        )}
        {clickCount > 20 && screen !== AppScreen.SUCCESS && (
          <p className="text-[#fbc02d] text-xl storybook-font italic font-bold drop-shadow-lg">
      ...
      <div className="fixed bottom-6 right-8 text-white/90 text-xl md:text-2xl storybook-font font-bold drop-shadow-[0_2px_4px_rgba(0,0,0,0.6)] z-20 hidden sm:block">
